...
public class FlockingManagerV2 : MonoBehaviour
{
    [System.Serializable]
    public class FlockV2
    {
        public bool waypointsAssigned = false;
        public List<FlockingAgentV2> agents = new List<FlockingAgentV2>();
    ...
    [SerializeField] private List<FlockV2> flocks = new List<FlockV2>();
    ...
    void Update()
    {
        foreach (FlockV2 flock in flocks)
        {
    ...
    public void CreateNewFlock(List<FlockingAgentV2> agents)
    {
        FlockV2 flock = new FlockV2();
        ...
        for (int i = 0; i < flocks.Count; i++)
        {
            for (int j = i + 1; j < flocks.Count; j++)
            {
                if (Vector3.Distance(flocks[i].flockCenter, flocks[j].flockCenter) < mergeRadius)
                {
                    ...
                    FlockingAgentV2 furthestAgent = FindFurthestAgent(flocks[i]);
    ...
    private FlockingAgentV2 FindFurthestAgent(FlockV2 flock)
    {
        FlockingAgentV2 furthestAgent = null; // Initialize furthestAgent to null
    ...
    private void CalculateFlockCenter(FlockV2 flock)
    {
    ...
    {
        List<FlockV2> newFlocks = new List<FlockV2>(); // List to store new flocks to add

        foreach (var flock in flocks)
        {
            List<FlockingAgentV2> agentsToRemove = new List<FlockingAgentV2>(); // List to store agents to remove
    ...
    public FlockV2 GetFlockFromAgent(FlockingAgentV2 agent)
    {
        foreach (FlockV2 flock in flocks)
        {
    ...
    public Vector3 CalculateFlockingBehaviors(FlockingAgentV2 agent, FlockV2 agentsFlock)
    {
